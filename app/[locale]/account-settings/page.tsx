            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="bg-white p-6 rounded-xl shadow-md mb-6">
                    ...
                    <div className="space-y-5">
                        <div>
                            <label className="block text-sm font-medium text-gray-500">{t('suiteNumber')}</label>
                            <p className="text-lg font-semibold text-gmc-gris-oscuro">{userData.suiteNumber}</p>
                        </div>
                        <div className="flex justify-between items-center">
                            <div>
                                <label className="block text-sm font-medium text-gray-500">{t('name')}</label>
                                <p className="text-lg text-gmc-gris-oscuro">{userData.name}</p>
                            </div>
                            <button onClick={() => setIsEditNameModalOpen(true)}><Edit size={18} className="text-gray-400 hover:text-gmc-dorado-principal" /></button>
                        </div>
                        <div className="flex justify-between items-center">
                            <div>
                                <label className="block text-sm font-medium text-gray-500">{t('country')}</label>
                                <p className="text-lg text-gmc-gris-oscuro">{userData.country}</p>
                            </div>
                            <button onClick={() => setIsEditCountryModalOpen(true)}><Edit size={18} className="text-gray-400 hover:text-gmc-dorado-principal" /></button>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">{t('language')}</label>
                            <LanguageSwitcher />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-md mb-6">
                    ...
                    <div className="space-y-5">
                        ...
                        {/* 🔥 TRADUCCIÓN: Checkbox SMS */}
                        <div className="flex justify-between items-center">
                            <label className="text-sm font-medium text-gray-700 pr-4">{t('sendSms')}</label>
                            <div className="relative inline-block w-10 mr-2 align-middle select-none">
                                <input type="checkbox" className="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer" checked={smsToggle} onChange={() => setSmsToggle(!smsToggle)}/>
                    ...
                    {hasAddress ? (
                        <div className="pb-5 border-b last:border-b-0">
                            ...
                            <div className="flex space-x-4 mt-3 text-sm font-medium">
                                <button onClick={() => setIsAddressModalOpen(true)} className="text-gmc-dorado-principal hover:underline">{t('btnEdit')}</button>
                                <button onClick={() => alert("No se puede borrar la dirección principal")} className="text-red-300 cursor-not-allowed">{t('btnDelete')}</button>
                            </div>
                        </div>
                    ) : (
                        <div className="text-center py-4">
                            ...
                            <button 
                                ...
                                className="w-full text-center py-2 bg-gray-100 text-gmc-gris-oscuro font-bold rounded-lg hover:bg-gray-200 flex items-center justify-center gap-2"
                            >
                                <Plus size={18} /> {t('btnAddAddress')}
                            </button>
                        </div>
                    )}
                </div>
