...
export default async function AdminDespachosPage() {
  ...
  // 🔥 CORRECCIÓN: Ahora busca 'POR_ENVIAR' y también 'PAGADO'
  const consolidacionesListas = await prisma.consolidatedShipment.findMany({
    where: { 
      status: { in: ['POR_ENVIAR', 'PAGADO'] } 
    },
    include: {
      user: true,
      packages: true
  ...
  // 🔥 CORRECCIÓN: Ahora busca todas las variantes de paquetes listos
  const paquetesSueltosListos = await prisma.package.findMany({
    where: {
      status: { in: ['POR_ENVIAR', 'LISTO_PARA_ENVIO', 'EN_PROCESO_ENVIO'] },
