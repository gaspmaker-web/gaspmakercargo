...
// --- DATA: Lista de países (Sincronizada con la detección de tu Backend) ---
const ALL_COUNTRIES = [
...
export default function CalculadoraClient() { // Renombrado para claridad interna
    ...
    const handleEstimate = async (e: React.FormEvent) => {
        ...
        // 2. Obtener Nombre del País para que el Backend use su "Omnisciente Detection"
        ...
        try {
            const res = await fetch('/api/rates', {
                ...
                body: JSON.stringify({
                    ...
                    destination: {
                        countryCode: country,
                        countryName: selectedCountry?.name || '', // Clave para la detección del backend
                        country: country
                    }
                })
            });
    ...
    // 🔥 CÁLCULO DEL DESGLOSE (Igual a la imagen ll140.jpg)
    ...
    return (
        <main className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 font-montserrat">
            ...
            {/* Modal de Resultados Reales (Con desglose como ll140.jpg) */}
            {isModalVisible && (
                    ...
                    <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-lg w-full overflow-hidden relative animate-in zoom-in-95">
                        ...
                        <div className="p-8 max-h-[60vh] overflow-y-auto space-y-4">
                            {apiRates.map((rate, index) => {
                                // 🔥 Cálculos en tiempo real en el cliente
