...
const AFFILIATE_STORES = [
  { id: 'amazon', name: 'Amazon', logo: 'https://www.amazon.com/favicon.ico', url: 'https://www.amazon.com/?tag=gaspmakercarg-20' },
  { id: 'ebay', name: 'eBay', logo: '/logos/ebay.svg', url: 'https://www.ebay.com/' },
  { id: 'shein', name: 'Shein', logo: 'https://us.shein.com/favicon.ico', url: 'https://us.shein.com/' },
  { id: 'walmart', name: 'Walmart', logo: 'https://www.walmart.com/favicon.ico', url: 'https://www.walmart.com/' },
  { id: 'temu', name: 'Temu', logo: 'https://www.temu.com/favicon.ico', url: 'https://www.temu.com/' },
  { id: 'aliexpress', name: 'AliExpress', logo: '/logos/aliexpress.webp', url: 'https://www.aliexpress.com/' },
];
...
const CopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button onClick={handleCopy} className={`ml-2 transition-colors p-1.5 rounded-md flex-shrink-0 ${copied ? 'bg-green-50 text-green-600' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`} title="Copiar">
...
const AddressRow = ({ label, value, isHighlighted = false }: { label: string, value: string, isHighlighted?: boolean }) => (
  <div className={`flex flex-col sm:flex-row sm:items-center py-2.5 sm:py-2 border-b border-gray-50 last:border-0 ${isHighlighted ? 'bg-blue-50/50 -mx-4 sm:-mx-6 px-4 sm:px-6 border-blue-100' : ''}`}>
    <span className="text-xs sm:text-sm font-medium text-gray-500 sm:w-32 md:w-36 lg:w-32 flex-shrink-0 mb-1 sm:mb-0">{label}:</span>
    <div className="flex items-center justify-between flex-grow w-full sm:w-auto overflow-hidden">
      <span className={`font-bold truncate mr-2 ${isHighlighted ? 'text-blue-800 text-base sm:text-lg' : 'text-gmc-gris-oscuro text-sm sm:text-base'}`}>{value}</span>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 items-stretch h-full">
      ...
      <div className="lg:col-span-5 bg-white p-4 sm:p-5 rounded-xl shadow-md border border-gray-100 flex flex-col justify-center overflow-hidden h-full">
        <div className="flex items-center gap-3 mb-3 pb-2 border-b border-gray-100">
          <div className="bg-blue-50 p-2 rounded-full text-blue-600 flex-shrink-0"><MapPin size={20} /></div>
          <h3 className="text-sm sm:text-base font-bold text-gmc-gris-oscuro font-garamond truncate">{t('title')}</h3>
        </div>
      ...
      <div className="lg:col-span-7 bg-white rounded-xl shadow-md border border-gray-100 flex flex-col overflow-hidden relative h-full">
        <div className="p-3 sm:p-4 border-b border-gray-50 bg-gradient-to-br from-blue-50/50 to-white shrink-0">
          ...
          <p className="text-[10px] sm:text-xs text-gray-500 mt-1 leading-tight">{t('affiliateDesc')}</p>
        </div>
        ...
        <div className="pt-4 px-2 sm:px-4 flex-1 flex flex-col justify-center bg-gray-50/30">
          <div className="flex overflow-x-auto gap-3 sm:gap-4 snap-x snap-mandatory pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            {AFFILIATE_STORES.map((store) => (
              ...
              >
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center bg-white border border-gray-100 shadow-sm overflow-hidden">
                  <img src={store.logo} alt={store.name} className="w-full h-full object-contain p-1"
                    onError={(e) => { const t = e.target as HTMLImageElement; t.style.display = 'none'; }}
                  />
                </div>
                <div className="text-center w-full mt-1">
                  <p className="font-bold text-gray-800 text-xs group-hover:text-blue-600 transition-colors truncate">{store.name}</p>
                  <span className="text-[9px] text-gray-400 font-bold uppercase flex items-center justify-center gap-1 mt-1 group-hover:text-blue-500">
                    <span>{t('buyNow')}</span>
