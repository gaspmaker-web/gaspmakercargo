...
export default function PackageDetailClient({
  ...
  const handlingFee = isGMCSelected ? 0.0 : pkg.handlingFee || 10.0;

  const declaredValue = Number(pkg.declaredValue) || 0;
  const insuranceCost = declaredValue > 100 ? declaredValue * 0.03 : 0;

  const warehousePhoto =
    pkg.photoUrlMiami && pkg.photoUrlMiami.startsWith("http")
      ? pkg.photoUrlMiami
      : null;

  const isReadyToShip =
    pkg.status === "RECIBIDO_MIAMI" || pkg.status === "EN_ALMACEN";
  ...
  useEffect(() => {
    if (discount > 0 && selectedRate) {
      if (selectedRate.price < 100) {
        ...
        setCouponMsg({
          type: "error",
          text: "Discount removed: Shipment value is under $100.",
        });
      }
    }
  ...
  const handleQuote = async () => {
    if (!hasInvoice) {
      alert("⚠️ " + t("invoiceRequired"));
      return;
    }
    if (!hasAddress) {
      alert("⚠️ " + tBills("errorAddress"));
      return;
    }
    if (!isAdminVerifiedValue) {
      alert("⚠️ Esperando validación del valor por el Admin.");
      return;
    }
    ...
    try {
      ...
      if (userProfile.countryCode?.toUpperCase() === "TT") {
        availableRates.unshift({
          ...
          service: "Air Freight (Trinidad Direct)",
          price: pkg.weightLbs * 2.5 + 10,
          currency: "USD",
          days: "3-5",
          logo: "/gaspmakercargoproject.png",
        });
      }
  ...
  const handleApplyCoupon = async () => {
    ...
    if (selectedRate.price < 100) {
      setTimeout(() => {
        setCouponMsg({
          type: "error",
          text: "Give $25, Get $25: Valid only on shipments over $100 USD.",
        });
    ...
    setTimeout(() => {
      ...
      setCouponMsg({
        type: "success",
        text: "Referral Credit applied! (-$25.00)",
      });
      setValidatingCoupon(false);
    }, 800);
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!payRes.ok)
        throw new Error(payData.message || "Error al procesar el cobro.");
  ...
  const totalAmount = Math.max(
    0,
    servicePriceWithFee + handlingFee + insuranceCost - discount,
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start pt-6">
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <span className="text-xl font-bold">{pkg.weightLbs}</span>{" "}
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <span className="text-lg font-bold">
                        {pkg.lengthIn}x{pkg.widthIn}x{pkg.heightIn}
                      </span>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div
                  className={`p-3 rounded-full ${hasInvoice ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-400"}`}
                >
                  {hasInvoice ? (
                    <CheckCircle size={24} />
                  ) : (
                ...
                <div>
                  <p className="text-sm font-bold text-gray-700">
                    {hasInvoice ? t("fileUploaded") : t("noInvoice")}
                  </p>
                  {hasInvoice && (
                    <a
                      href={invoiceUrl!}
              ...
              <label className="cursor-pointer bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-xs font-bold hover:bg-gray-50 flex items-center gap-2">
                ...
                {hasInvoice ? t("replaceBtn") : t("uploadBtn")}
                <input
                  type="file"
              ...
              <div className="bg-blue-50 p-5 border-b border-blue-100 flex flex-col sm:flex-row justify-between items-start gap-4">
                ...
                {isReadyToShip &&
                  ...
                  (isOverdue ? (
                    ...
                    <div className="bg-yellow-50 text-yellow-800 px-6 py-3 rounded-xl text-sm font-bold border border-yellow-200 flex items-center gap-3 w-full sm:w-auto animate-pulse">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-normal opacity-80">
                          El almacén debe confirmar el valor.
                        </span>
                      </div>
                    </div>
                  ) : (
                    <button
                      onClick={handleQuote}
                      disabled={loadingRates || !hasAddress}
                      className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 transition shadow-md flex items-center justify-center gap-2"
                    >
                      {loadingRates ? (
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div
              className="sticky top-6 rounded-2xl p-6 shadow-2xl text-white transition-all duration-500 animate-slideDown"
              ...
              {selectedRate ? (
                <>
                  <div className="space-y-4 border-b border-gray-600 pb-6 mb-6">
                    <div className="flex justify-between text-sm">
                      <span>
                        Freight & Processing (
                        {cleanCarrierName(selectedRate.carrier)})
                      </span>
                    ...
                    <div className="pt-2">
                      ...
                      {couponMsg.text && (
                        <p
                          className={`text-[10px] mt-2 font-bold flex items-center gap-1 ${couponMsg.type === "error" ? "text-red-400" : "text-green-400"}`}
                        >
                  ...
                  <div className="mb-6">
                    ...
                    {cards.length > 0 ? (
                      <div className="space-y-3">
                        <div className="relative bg-gray-700 rounded-lg border border-gray-600 hover:border-gray-500">
                          ...
                          <select
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-transparent text-white text-sm font-bold outline-none cursor-pointer appearance-none"
                          >
                            {cards.map((c: any) => (
                              <option
                                key={c.id}
                        ...
                        <button
                          ...
                          className="text-xs text-[#EAD8B1] hover:underline flex items-center gap-1"
                        >
                      ...
                      >
                        <div className="bg-gray-800 p-1 rounded group-hover:bg-gray-900 transition">
                          <Plus size={16} />
                        </div>
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-xl text-lg font-bold shadow-lg hover:brightness-110 transition-all flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-[#222b3c]"
                    style={{ backgroundColor: "#EAD8B1" }}
                  >
                    {isPaying ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm px-4">
                    Selecciona un courier a la izquierda para ver el total a
                    pagar.
                  </p>
                </div>
              )}
            ...
            <div className="bg-[#222b3c] rounded-t-3xl shadow-[0_-5px_25px_rgba(0,0,0,0.2)] p-5 animate-slideUp text-white">
              <div className="flex justify-between items-center gap-4">
                ...
                >
                  ...
                  <div className="text-3xl font-garamond font-bold leading-none text-white">
                    ${totalAmount.toFixed(2)}
                  </div>
                </div>
                <button
                ...
                <div className="mt-5 pt-5 border-t border-gray-600 space-y-3 text-sm animate-fadeIn">
                  ...
                  <div className="pt-3">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={handleApplyCoupon}
                        className="bg-gray-600 text-white px-3 rounded-lg text-xs hover:bg-gray-500"
                      >
                        Apply
                      </button>
                    </div>
                    {couponMsg.text && (
                      <p
                        className={`text-[10px] mt-1 ${couponMsg.type === "error" ? "text-red-400" : "text-green-400"}`}
                      >
                        {couponMsg.text}
                      </p>
                    )}
                  ...
                  <div className="pt-4 border-t border-gray-600">
                    ...
                    {cards.length > 0 ? (
                      <div className="space-y-2">
                        <div className="relative bg-gray-700 rounded-lg border border-gray-600">
                          <select
                            value={selectedCardId}
                            onChange={(e) => setSelectedCardId(e.target.value)}
                            className="w-full p-2.5 bg-transparent text-white text-sm font-bold outline-none"
                          >
                            {cards.map((c: any) => (
                              <option
                                key={c.id}
                        ...
                        <button
                          onClick={handleAddCardRedirect}
                          className="text-xs text-[#EAD8B1] font-bold flex items-center gap-1 hover:underline"
                        >
                      ...
                      <button
                        onClick={handleAddCardRedirect}
                        className="w-full py-2 bg-gray-700 text-white font-bold rounded-lg border border-gray-600 flex items-center justify-center gap-2"
                      >
